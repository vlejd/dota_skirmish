...
--		self:SetForwardVector(self.ForwardVector)
	...
	if (self:GetParent():GetAbsOrigin() - ROSHAN_SPAWN_LOC):Length2D() >= self.leash_distance then
		self.returningToLeash = true
		self:GetParent():MoveToPosition(ROSHAN_SPAWN_LOC)
	end
end
...
function modifier_imba_roshan_ai:OnDeath( keys )
    ...
    r:AddItemByName("item_aegis")
    r:DropItemAtPositionImmediate(r:GetItemInSlot(0), r:GetCenter())
	...
	--local respawn_time = RandomInt(8, 11) * 60
	--Timers:CreateTimer(respawn_time, function()
	--	local ROSHAN_ENT = CreateUnitByName("npc_dota_roshan_custom", ROSHAN_SPAWN_LOC, true, nil, nil, DOTA_TEAM_NEUTRALS)
